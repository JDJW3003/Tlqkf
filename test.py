...
async def on_message(message):
    ...
    if message.content == ".이정원":
        await message.channel.send("이정원 없지이정원 없지이정원 없지이정원 없지이정원 없지")
        await message.channel.send("이정원 진짜 뭐함이정원 진짜 뭐함이정원 진짜 뭐함이정원 진짜 뭐함")
    if message.content == ".":
        await message.channel.send("뭘 또 . 이야 ㅋ")
    ...
    if message.content == "?롤덤이 추첨 안내":
        embed = discord.Embed(title="롤랜덤이 사용방법", description=" :bulb: Tip : 밑에 있는 도움말을 보고 이용해주세요 :D", color=0x17164D)
        embed.add_field(name=":newspaper:전체 추첨 명령어", value=" **```?롤덤이 전체 추천```** ", inline=False)
        embed.add_field(name="포지션 별 추첨 ", value="**```?롤덤이 암살자 추천```**", inline=False)
        embed.add_field(name="_", value="**```?롤덤이 메이지 추천```**", inline=False)
        embed.add_field(name="_", value="**```?롤덤이 서포터 추천```**", inline=False)
        embed.add_field(name="_", value="**```?롤덤이 전사 추천```**", inline=False)
        embed.add_field(name="_", value="**```?롤덤이 탱커 추천```**", inline=False)
        embed.set_footer(text="문의는 discordbotgang#0010")
        await message.channel.send(embed=embed)
